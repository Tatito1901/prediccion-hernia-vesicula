...
import { AlertTriangle, TrendingUp, Users, Activity, Percent, BarChartHorizontal, Clock } from "lucide-react";
...
// Función pura para transformar datos del chart
function transformChartData(chart: any): ChartDataPoint[] {
  if (!chart?.series || !chart.categories || chart.series.length === 0) {
    return [];
  }

  const consultasIndex = chart.series.findIndex((s: any) => s.name === 'Consultas');
  const operadosIndex = chart.series.findIndex((s: any) => s.name === 'Operados');
...
// Función pura para calcular métricas
function calculateMetrics(clinicMetrics: any, chartData: ChartDataPoint[]) {
  const defaults = {
    totalConsultas: 0,
    totalOperados: 0,
    conversionRate: 0,
  ...
  if (clinicMetrics) {
    return {
      totalConsultas: clinicMetrics.totalPacientes || 0,
      totalOperados: clinicMetrics.pacientesOperados || 0,
      conversionRate: Math.round((clinicMetrics.tasaConversion || 0) * 100),
      avgDecisionTime: clinicMetrics.tiempoPromedioDecision || 0,
      mainSource: clinicMetrics.fuentePrincipalPacientes || 'No disponible'
    };
  }

  if (chartData.length > 0) {
    const totalConsultas = chartData.reduce((sum, item) => sum + item.consultas, 0);
    const totalOperados = chartData.reduce((sum, item) => sum + item.operados, 0);
    const conversionRate = totalConsultas > 0 ? Math.round((totalOperados / totalConsultas) * 100) : 0;
    return { ...defaults, totalConsultas, totalOperados, conversionRate };
  }

  return defaults;
}

// Mapa de descripciones de rango de fechas
...
function MetricCard({ icon: Icon, label, value, trend, colorClass, description }: MetricCardProps) {
  return (
    ...
    <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-3xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        {Array.from({ length: 3 }).map((_, i) => (
          <Card key={i} className="bg-white/60 dark:bg-slate-800/70 backdrop-blur-sm rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <CardContent className="p-5">
    ...
    <Card className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-3xl">
      <CardContent className="flex flex-col items-center justify-center h-[500px] text-center p-8">
        ...
        <p className="text-sm text-amber-700 dark:text-amber-400 max-w-md">
          No pudimos recuperar la información de analíticas. Por favor, intenta refrescar la página.
        </p>
...
export const PatientAnalytics: React.FC = () => {
  ...
  const metrics = calculateMetrics(clinicMetrics, chartData);
  const dateRangeLabel = DATE_RANGE_MAP[dateRange as keyof typeof DATE_RANGE_MAP] || 'Todos los datos';
